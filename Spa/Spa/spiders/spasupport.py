...
class Spapart(scrapy.Spider):
    ...
    def parse_product(self, response):
        ...
        if response.css("div.stock-info"):
            stock_status = response.css(" div.stock-info span.themify-child-stock-status::text").get().strip()
            if stock_status == "Lagervara":
                item['stock_status_refined'] = 'IN_STOCK'
            elif stock_status == "Slut i lager":
        ...
        specs = [li.css('::text').get().strip() for li in response.css("div.product-description ul li")]
